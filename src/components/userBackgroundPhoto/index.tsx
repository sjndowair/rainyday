...
const UserBackgroundPhoto = ({isUserState}:IUserBackgroundPhotoProps) => {
    ...
    const user = isUserState?.uid;
        ...
        const {isFileInputRef, isFetchImageFile, setIsUserImage,
            isUserImage,
            isHandleImageChange
            ,isHandleEditClick} = useFireBaseImage({userId: user || null , collectionData: "background"})

    useEffect(() => {
        const fetchUserData = async () => {
            try {
                if (isUserState?.uid) {
                    const userDoc = await getDoc(doc(db, "background", isUserState.uid));
