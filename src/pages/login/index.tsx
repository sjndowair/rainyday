...
type TErrorCode = 'auth/cancelled-popup-request' | 'auth/popup-closed-by-user' | 'auth/popup-blocked' | "auth/invalid-email" | "auth/user-disabled" | "auth/user-not-found" | "auth/wrong-password";

const ERROR_KIND = {
    "auth/cancelled-popup-request": "팝업이 열려있습니다. 팝업을 닫고 다시 시도해주세요",
    "auth/popup-closed-by-user": "인증완료가 되기전에 팝업이 닫혔습니다. 다시 인증해주세요",
...
const Login = ()=> {
        ...
        try {
           ...
           console.log("User signed in Error:", error);
            if (typeof error === 'object' && error !== null && 'code' in error && typeof error.code === 'string' && 'message' in error && typeof error.message === 'string') {
                setIsErrorMessage( getErrorMessage(error as { code: string; message: string }))
            }
            isUnknownErrorBox();
    ...
    const isOnClickSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const { user } = await signInWithEmailAndPassword(auth, isInputData.email, isInputData.password);
                console.log(user)
                navigate("/");
        } catch (error: unknown) {
            setIsLoading(true);
          ...
          <CenterBox>
            ...
            <MembershipInnerContain>
              <Form isHandleSubmit={isOnClickSignIn}>
                ...
                <div className={`pt-1`} />
                  <Button >로그인</Button>
                  <Button onClick={isOnClickGoogleSignIn}>구글계정으로 로그인하기</Button>
              </Form>
                <div className="pt-5" />
